...
def load_preprocessor(
    ...
    scaler = ColumnTransformer(
        [("std", StandardScaler(), numeric_cols)], remainder="passthrough"
    )
    cat_encoder = load_cat_encoding(name=cat_encoder)
    steps = [DropFeatures(cols_to_drop), DropConstantFeatures(), cat_encoder, scaler]

    if poly_degree > 0:
        step = PolynomialFeatures(
            degree=poly_degree, interaction_only=add_interaction_only
        )
        steps.append(step)
